...
### 글로벌 동조성 — 미국 반도체 종목/ETF 매핑 (★ KIS Open API 활용)

한국 반도체는 미국 시장 동조성이 매우 높습니다 (DRAM/HBM·파운드리 50~70%, 장비 60~80%). **미국 반도체 종목·ETF의 야간 종가가 다음 영업일 한국 반도체 시장의 선행지표**로 작용하므로, KIS 해외주식 API로 직접 조회해 분석 정확도를 크게 높입니다.

#### 핵심 미국 반도체 카탈로그 (한국 매핑)

| 미국 티커 | 거래소 | 한국 매핑 | 동조 채널 | 우선순위 |
|---|---|---|---|:-:|
| **SOXX** | NAS | 반도체 섹터 전체 | iShares Semiconductor ETF — 글로벌 반도체 강도 1차 지표 | ★★★ |
| **SMH** | NAS | 반도체 섹터 전체 | VanEck Semiconductor ETF — 거래량 1위, 보조 검증 | ★★★ |
| **NVDA** | NAS | SK하이닉스(HBM3E), 삼성(GDDR), 이수페타시스(GPU PCB) | **AI GPU 수요 → HBM 직접 수요** | ★★★ |
| **TSM** | NYS | DB하이텍(8인치 파운드리), 가온칩스·에이직랜드(VCA) | **파운드리 사이클 + Custom ASIC 글로벌 베이스라인** | ★★★ |
| **MU** | NAS | 삼성전자·SK하이닉스 (DRAM/HBM 가격) | **메모리 사이클 동조** (HBM 3사 중 미국 1사) | ★★★ |
| **ASML** | NAS | 원익IPS·주성엔지니어링·HPSP·한미반도체 (간접) | **EUV/WFE 사이클** | ★★ |
| **AVGO** | NAS | 가온칩스, 에이직랜드, 이수페타시스 | **Custom ASIC + 하이퍼스케일러 자체칩 수혜** | ★★ |
| **AMD** | NAS | SK하이닉스 (MI300/350 HBM3E), 이수페타시스 | **AI GPU 2위 → HBM3E 수요** | ★★ |
| **AMAT** | NAS | 원익IPS, 주성엔지니어링 (증착) | WFE 1위 — 한국 장비주 동조 | ★★ |
| **KLAC** | NAS | 파크시스템스 (검사), 코미코 (세정 부품) | 검사·계측 장비 | ★★ |
| **LRCX** | NAS | 피에스케이(Strip), 주성엔지니어링(ALD) | 식각·증착 장비 | ★★ |
| **TER** | NAS | 디아이, 엑시콘, 테크윙, 이오테크닉스 | **테스터 동조** | ★★ |
| **INTC** | NAS | 삼성전자 파운드리 비교 (18A vs SF2) | 파운드리 2위 추격 | ★ |
| **SNPS** | NAS | 가온칩스, 에이직랜드 | EDA — 디자인하우스 파이프라인 | ★ |
| **CDNS** | NAS | 가온칩스, 에이직랜드 | EDA — 디자인하우스 파이프라인 | ★ |

#### 동조성 시그널 매트릭스

| SOXX/SMH 야간 등락 | NVDA 야간 등락 | TSM/MU 야간 등락 | 다음날 한국 시그널 |
|:-:|:-:|:-:|---|
| > +3% | > +3% | > +2% | **글로벌 반도체 강세 → 한국 메모리/HBM/장비 매수세 강함** |
| +1~3% | +1~3% | +1~2% | 완만한 강세, 갭상승 출발 가능 |
| ±1% | ±1% | ±1% | 횡보, 매크로 트리거 대기 |
| -1~-3% | -1~-3% | -1~-2% | 갭하락 출발 가능, 1차 지지선 점검 |
| < -3% | < -3% | < -2% | **글로벌 반도체 약세 → 한국 메모리/HBM/장비 동반 약세** |
| **NVDA 강세 + 한국 HBM 약세** | - | - | 디버전스: HBM4 qualification 이슈 가능성 (SK하이닉스/삼성) |
| **TSM 강세 + 한국 파운드리 약세** | - | - | 디버전스: 삼성 파운드리 점유율 격차 확대 신호 |
| **MU 약세 + 한국 DRAM 강세** | - | - | 디버전스: 한국 메모리 단독 수혜 (가격 격차 또는 환율) |
...
### 2.5단계: KIS Open API로 글로벌 반도체 종목/ETF 야간 종가 수집 (★ 한국 개장 선행지표)

한국 반도체는 미국 시장 동조성이 50~80%로 매우 높습니다. KIS 해외주식 API로 미국 반도체 종목·ETF 야간 종가를 직접 수집하여 다음날 한국 반도체 시장의 선행지표로 활용합니다. 본 단계는 MCP가 아닌 **bash 직접 실행**입니다.

#### 실행 (Tier 1 — 핵심 8개, ★★★ 우선)
```bash
cd /Users/cody/stocks/apps/server && npx tsx scripts/fetch-kis-prices.ts \
  --overseas NAS:SOXX,NAS:SMH,NAS:NVDA,NYS:TSM,NAS:MU,NAS:ASML,NAS:AVGO,NAS:AMD \
  --json 2>/dev/null
```

#### 실행 (Tier 2 — 장비/EDA 7개, ★★ 옵션)
분석 초점이 "전공정장비"/"후공정"/"디자인하우스"/"AI반도체"일 때 추가 호출:
```bash
cd /Users/cody/stocks/apps/server && npx tsx scripts/fetch-kis-prices.ts \
  --overseas NAS:AMAT,NAS:KLAC,NAS:LRCX,NAS:TER,NAS:INTC,NAS:SNPS,NAS:CDNS \
  --json 2>/dev/null
```

#### 응답 활용

응답 JSON: `{ "overseas": { "SOXX": {last, changePct, volume, ...}, "NVDA": {...}, ... } }`

**1) 동조성 시그널 산출**:
- SOXX/SMH 평균 등락률 → 글로벌 반도체 강도
- NVDA 등락률 → HBM 직접 수요 신호
- TSM 등락률 → 파운드리 사이클 + Custom ASIC 베이스라인
- MU 등락률 → DRAM/HBM 메모리 가격 선행
- ASML 등락률 → 전공정 장비주 선행

**2) 동조성 시그널 매트릭스 적용** (도메인 §"동조성 시그널 매트릭스" 참조)
- 다음날 한국 반도체 갭상승/갭하락 확률 예상
- 디버전스 발생 종목군 식별 (예: NVDA 강세 + SK하이닉스 약세 → HBM4 qualification 이슈)

**3) 종목별 매핑 분석**:
- NVDA 야간 등락률 → SK하이닉스·이수페타시스 갭 예측
- TSM 야간 등락률 → 가온칩스·에이직랜드 갭 예측
- MU 야간 등락률 → 삼성전자·SK하이닉스 메모리 라인 예측
- TER 야간 등락률 → 디아이·엑시콘·테크윙 테스터주 예측

**4) 리포트 활용**:
- 6단계 리포트의 "글로벌 반도체 야간 신호" 섹션에 반영
- 핵심 종목 상세 분석 시 "글로벌 동조 종목 야간 등락률"을 보조 신호로 인용

#### 주의사항
- KIS 응답 `status: empty/error`인 종목은 동조성 산출에서 제외 (typically SOXX/SMH/NVDA는 안정적)
- 미국 시장 휴장 시 전일 종가 반영
- 미국 정규장(KST 22:30~05:00) 마감 후 호출 시 가장 정확
- 한국 정규장 시간(09:00~15:30)에 호출하면 전일 미국 종가 반영
- 레이트리밋(220ms 딜레이) 적용으로 Tier 1만 호출 시 ~2초, Tier 1+2 호출 시 ~4초 소요
...
3.5. **글로벌 반도체 야간 신호** (★ 2.5단계 KIS 데이터 활용)
   - **SOXX/SMH 야간 등락률** + 평균 (글로벌 반도체 강도)
   - **NVDA 야간 등락률** — HBM 직접 수요 신호 → SK하이닉스/이수페타시스 갭 예측
   - **TSM 야간 등락률** — 파운드리 사이클 + Custom ASIC 베이스라인 → 가온칩스/에이직랜드/DB하이텍 갭 예측
   - **MU 야간 등락률** — 메모리 가격 선행 → 삼성전자/SK하이닉스 메모리 라인 갭 예측
   - **ASML 야간 등락률** — 전공정 장비주 선행 → 원익IPS/주성엔지니어링/HPSP 갭 예측
   - **AVGO/AMD 야간 등락률** — Custom ASIC/AI GPU 2위 신호
   - **장비/EDA 야간 등락률** (Tier 2 호출 시): AMAT/KLAC/LRCX/TER/SNPS/CDNS
   - **동조성 시그널 매트릭스 적용**: 글로벌 강세/약세/디버전스 판정 (도메인 §"동조성 시그널 매트릭스" 참조)
   - **종목별 갭 예측 매트릭스**: 미국 종목 등락률 × 한국 매핑 종목별 다음날 갭 예상 (시그널 정합성: 강세 다이렉트 매핑 시 +0.5~1.5%p / 약세 동일)
...
- **글로벌 반도체 야간 신호 1줄** (KIS 데이터): SOXX/SMH/NVDA/TSM/MU/ASML 평균 등락률 + 한국 다음날 갭 예측 (강세/약세/디버전스)
